...
| S21 | §§21.1–21.5, §21.12, §§21.14–21.15 | FIIR, C import and wrapper/thunk generation, recipes, provenance, and support diagnostics | Export-side C11 products are complete. Import-side C `_Bool`, exact IEEE binary16 `_Float16`, exact IEEE binary32 `float`, exact IEEE binary64 `double`/`long double`, every fundamental C integer family, scalar typedef chains, named/typedef-backed C enums, constant-only anonymous enums, and plain nested records pass Clang facts → validated/serialized FIIR → generated raw/C products → both semantic oracles and real QBE/C execution. Scalar families and typedef names remain nominal; `_Float16` uses an exact private f32 carrier; enum values and anonymous constants use target-neutral sign-magnitude carriers; records use logical fields while layout stays in FIIR/C. Extended `long double` fails rather than narrowing. Integer target-range failure traps before C, while alternate primitive/typedef targets, enum representation flags, and record layout facts preserve identical Luce semantics. Other extended floating carriers, unions/bit-fields, macro/object constants, ownership recipes, support tiers, and regeneration diagnostics remain; C++ §§21.6–21.11 and §21.13 are deferred past 1.0. |
