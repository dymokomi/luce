...
| §23 | Semantic pipeline and runtime services | complete | partial | partial | partial | partial | Worker services, source traces/fatal outcomes, source-test integration of resource reports, self-hosting, and the final full-language QBE artifact proof remain. Backend portfolio, artifact, ABI, profile, and cache policy are explicitly post-1.0. |
...
| S30 | §§22–23 | Finish the standard modules and runtime services required by the proving examples, then run the full corpus through QBE | Core runtime storage/collections/text/classes are executable; workers, codecs, host libraries, and source-test resource-report integration remain. |
