...
| `base/c_arithmetic.lucb` | C's `//` and `%`, implicit same-signedness widening, and the `+% +| +?` overflow family in a Luce Base module | HIR execution and a freestanding Wasm build through the pipeline; the same arithmetic floors in a `.luc` module (base.md §7.2, §7.5). |
