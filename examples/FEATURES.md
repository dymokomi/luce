...
| `base/format.lucb` | `print(f"...")` over integers, `bool`, `char`, `str`, and `u32(letter)` | Built natively under QBE and run: each piece reaches standard output through the standard `io` module's display functions, with no string value formed (base.md §5.5, §14.4). |
