...
| `base/atomics.lucb` | `@u64` counters with `+=`, `add(1, .relaxed)`, `max`, `min`, `swap`, and `cas`, an `@bool` flag with `store`/`load` orders | HIR execution and a freestanding Wasm build through the pipeline; the same fixture runs natively under QBE through `__atomic_*` calls (base.md §15.1, §19.3). |
