...
| §21 | C/native imports and exports, ownership/nullability, raw native source | complete | partial | partial | partial | partial | Scalar/handle/nested external and exported structs, strings, borrowed dense lists, anonymous `foreign` data pointers, incoming bare/nullable cfunc pointers, and explicit inbound-memory copies retain target-neutral HIR/MIR shape, with real QBE/C proofs. Standalone C11 export products execute through the CLI. Clang emits versioned FIIR plus generated nominal fundamental, scalar-typedef, and sign-magnitude enum carriers, while constant-only anonymous enums become typed constants; checked C adapters compile, link, run, assert exact scalar representations, reject target-range overflow, and reject undeclared enum values without target facts entering HIR/MIR. Extended floats and broader declarations, recipes, f16, and callback runtime/lifetime rules remain. |
...
| S21 | §§21.1–21.5, §21.12, §§21.14–21.15 | FIIR, C import and wrapper/thunk generation, recipes, provenance, and support diagnostics | Export-side C11 products are complete. Import-side C `_Bool`, exact IEEE binary32 `float`, exact IEEE binary64 `double`/`long double`, every fundamental C integer family, scalar typedef chains, named/typedef-backed C enums, constant-only anonymous enums, and plain nested records pass Clang facts → validated/serialized FIIR → generated raw/C products → both semantic oracles and real QBE/C execution. Scalar families and typedef names remain nominal; enum values and anonymous constants use target-neutral sign-magnitude carriers; records use logical fields while layout stays in FIIR/C. Extended `long double` fails rather than narrowing. Integer target-range failure traps before C, while alternate primitive/typedef targets, enum representation flags, and record layout facts preserve identical Luce semantics. Extended floating carriers, unions/bit-fields, macro/object constants, ownership recipes, support tiers, and regeneration diagnostics remain; C++ §§21.6–21.11 and §21.13 are deferred past 1.0. |
| S22 | §21.16 | Raw memory copy/take verbs and the f16 generated shim | `c.bytes_at`, `c.cstring_at`, and `c.take_str` execute through both semantic oracles and real linked QBE/C; the generated f16 adapter waits for S21 FIIR, and automatic standard-module product loading remains in S30. |
...
| `c_import/` | Generated C import architecture | C Boolean, integer, IEEE float/double, scalar typedef, named/typedef-backed enumeration values, constant-only anonymous-enum values, and simple/nested plain records pass Clang/FIIR generation, HIR and MIR oracles, Wasm/QBE encoding, and an end-to-end `luce bind` → native build → linked C execution gate. Record layout remains Clang/FIIR/C-only. Remaining FIIR vocabulary is tracked by S21. |
