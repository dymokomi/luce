...
| §15 | Generic declarations, constraints, monomorphization, limits | complete | partial | partial | partial | partial | Generic functions, memberwise generic structs/enums/classes, owner-parameterized and independently generic instance methods, concrete conformances, interface constraints/intersections, abstract body checking, structural inference, contextual values, defaults, recursion, memoized instances, and an explicit generator specialization budget reach QBE; custom generic-nominal initializers/type functions, serialized typed bodies, package/CLI budget configuration, origin/size/time accounting, and expansion-path diagnostics remain. |
