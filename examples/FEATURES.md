...
| §21 | C/native imports and exports, ownership/nullability, raw native source | complete | partial | partial | partial | partial | Scalar/handle/nested external and exported structs, strings, borrowed dense lists, anonymous `foreign` data pointers, incoming bare/nullable cfunc pointers, and explicit inbound-memory copies retain target-neutral HIR/MIR shape, with real QBE/C proofs. Standalone C11 export products execute through the CLI. Clang emits versioned FIIR plus generated nominal `c.boolean`, `c.float`, `c.double`, `c.long_double`, and fundamental-integer carriers; checked C adapters compile, link, run, assert exact C representations, and reject target-range overflow without target facts entering HIR/MIR. Extended floats/typedefs/declarations, recipes, f16, and callback runtime/lifetime rules remain. |
...
| S21 | §§21.1–21.5, §21.12, §§21.14–21.15 | FIIR, C import and wrapper/thunk generation, recipes, provenance, and support diagnostics | Export-side C11 products are complete. Import-side C `_Bool`, exact IEEE binary32 `float`, exact IEEE binary64 `double`/`long double`, and every fundamental C integer family pass Clang facts → validated/serialized FIIR → generated raw/C products → both semantic oracles and real QBE/C execution. Families remain nominal when representations coincide; extended `long double` fails rather than narrowing. Integer target-range failure traps before C, while alternate integer and `_Bool` widths produce byte-identical Luce source. Extended floating carriers, typedefs/declarations, ownership recipes, support tiers, and regeneration diagnostics remain; C++ §§21.6–21.11 and §21.13 are deferred past 1.0. |
