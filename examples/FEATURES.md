...
| S21 | §§21.1–21.5, §21.12, §§21.14–21.15 | FIIR, C import and wrapper/thunk generation, recipes, provenance, and support diagnostics | Export-side C11 products are complete. Import-side C `_Bool`, exact IEEE binary32 `float`, exact IEEE binary64 `double`/`long double`, every fundamental C integer family, scalar typedef chains, named/typedef-backed C enums, and constant-only anonymous enums pass Clang facts → validated/serialized FIIR → generated raw/C products → both semantic oracles and real QBE/C execution. Scalar families and typedef names remain nominal; enum values use one target-neutral sign-magnitude shape and undeclared results trap; anonymous constants retain their exact C type only in FIIR and share one target-neutral sign-magnitude source carrier. Extended `long double` fails rather than narrowing. Integer target-range failure traps before C, while alternate primitive/typedef targets and enum representation flags preserve identical Luce semantics. Extended floating carriers, records/unions, macro/object constants, ownership recipes, support tiers, and regeneration diagnostics remain; C++ §§21.6–21.11 and §21.13 are deferred past 1.0. |
...
| `c_import/` | Generated C import architecture | C Boolean, integer, IEEE float/double, scalar typedef, named/typedef-backed enumeration values, and constant-only anonymous-enum values pass Clang/FIIR generation, HIR and MIR oracles, Wasm/QBE encoding, and an end-to-end `luce bind` → native build → linked C execution gate. Remaining FIIR vocabulary is tracked by S21. |
