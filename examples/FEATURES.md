...
| B1 | base.md §§1–4, 5.1–5.2, 7.2–7.5, 16.2, 19.1 | The Base profile: suffix selection, `.lucn` rename, Base tokens and reserved words, tier rejections, freestanding reachability, `usize`/`isize`, implicit widening, C division, the cast family | Partial (2026-09-03): `.lucb` selects the Base profile, the audited tier is `.lucn`, a Base module imports only Base modules, every runtime-backed spelling (classes, collections and their literals, `new`, `spawn`, `wait_all`, `Weak`, capturing lambdas, block closures) is refused naming the tier, and a Base package is checked freestanding after lowering; `examples/base/hello.lucb` checks, runs, and builds. The Base spellings (`+% -% *% +| -| *| +? -? *?` and their augmented forms, `---`, `...`, `@`) and the Base reserved words are admitted by the one lexer only in `.lucb` modules and named in a diagnostic elsewhere; their grammar lands with the slices that give them meaning. `usize`/`isize` resolve only in `.lucb` modules as one widthless canonical type that the layout widens (32 bits in the oracle's 4-byte rules and on wasm32, 64 under QBE), with constants that fit only the wider target refused by name where the width is known; `examples/base/pointer_width.lucb` checks, runs, and builds. `sizeof`/`alignof`/`offsetof` check as `usize` and travel through HIR and MIR as a symbolic `LayoutConstant` that each backend, the reference interpreter included, folds from its own layout rules. `//` and `%` truncate toward zero in `.lucb` modules and an integer widens implicitly to a wider one of the same signedness, including to and from the pointer width through the canonical `widen` conversion; `examples/base/c_arithmetic.lucb` checks, runs, and builds. The operator family `+% -% *%` (and unary `-%`), `+| -| *|`, and `+? -? *?` wraps, saturates, or answers `none` on overflow through both oracles and both backends. `(T)x` is C's cast between numbers: low bits, saturating float truncation, IEEE rounding, never a trap. Open: pointer, enum, and text casts (with their types), and folding `usize` constants into array lengths and module-level asserts. |
| B2 | base.md §§5.3–5.5, 6.6, 7.7, 8.3 | Pointers, qualifiers, the null niche, spans, `str` views, `cstr`, address-of and the escape rule, pointer arithmetic and ordering, `for x in &items` | Partial (2026-09-03): `T*`, `const T*`, `&place` with the path-rooted qualifier, `*p` loads and stores, `p.field` auto-dereference, implicit `T*` to `const T*`, and address equality check, run through the reference interpreter's frame-place model, lower to canonical `Ptr`, and execute on QBE and Wasm; `examples/base/pointers.lucb` checks, runs, and builds; `T*?` is the one-word null niche in canonical MIR, unwrapped like any optional; `p + n`, `p[i]`, `p - q`, address ordering, `void*`, and the pointer casts `(U*)p`, `(usize)p`, `(T*?)n` follow C; `T[N]` arrays and `T[]` spans with checked indexing and slicing, `length`/`data`, `T[](pointer, count)`, array-to-span conversion, and `for` over both by value or, with `&items`, by element pointer; the escape rule refuses an address of a local that would outlive its frame; `str` is a two-word view with `length`, `bytes`, byte comparison, `(str)bytes`, and `for character in text` decoded inline. Open: `cstr` (with the `c` module, B4). |
| B3 | base.md §§6.1–6.5, 8.4–8.8, 10.3–10.4, 11.7, 12 | Zero values, `---`, globals, unions, integer-backed enums, `packed`/`align`, `new`/`alloc`/`free`/`with`/`in`, `Allocator`, `errdefer`, labels, guards | Complete (2026-09-04): a `var` with a written type and no initialiser holds its zero (`ZeroValue`, canonical `Zero`), `= ---` allocates without writing, and a type holding a bare pointer or function is refused by name; `enum Access as u32:` is its representation in MIR and both oracles, with `\|` `&` `^` `~` producing the enum, `(u32)e` and `(Access)n` as casts, `Access(n)` the checked conversion (`EnumFromInteger`), and `match` requiring `_`; `union Name:` with `name: type` members overlaps them at one address (`HirRecordLayout.overlapping`, canonical `Union`), construction writes exactly one member, reads reinterpret bytes in MIR and both oracles, and the type has no equality or interfaces; module `var` and `thread_local var` with constant initialisers (`HirGlobal.initializer`, folded by `ConstantFolder` into the canonical `MirInitializer` value tree that each backend lays out) and `pub let` constants; labeled loops with `break label`/`continue label` (`BreakStatement(levels)`), match guards `pattern if condition:` that bind first and never count toward exhaustiveness, and `errdefer` (`ErrorDeferStatement`, run only on an error exit by the lowerer's `ErrorSource` and the oracle). Open: `packed`/`align`, allocation; the C header form of an integer-backed enum waits for B4 (§17.6); a global initialiser that puts a layout answer inside an expression, a text view, or the address of a global's part is refused with a diagnostic for now; the Base failure model (§11.2–11.4) gives a `.lucb` module `Error` and `ErrorCode` whose text is a `str` view, so `error(code, "text")`, `try`, `catch`, `recover`, `else error(...)`, and `errdefer` run without the runtime in both oracles, Wasm, and native QBE; `packed struct`, `align(N) struct`, and `align(N)` before a field (`HirStruct.is_packed`/`alignment`, `MirField.alignment`/`is_packed`, laid out by every backend's `field_align`) with the packed-field address rule; allocation (§12) through the standard `memory` module's `Allocator` interface, where `new T(...)`, `new T`, `new T[n]`, `alloc T[n]`, and `alloc(size, align)` are the HIR `Allocate` node whose request is an interface call on the chosen allocator, `free(x)` is its `release` call, `with a:` (`WithAllocator`) swaps the thread-local `memory.allocator` view and restores it on every exit, `in a` names the allocator, and exhaustion is the recoverable `memory.exhausted`; `FixedBuffer` is the first allocator. The linter rules of §12.5 and the `PageAllocator`/`CAllocator`/`Arena` allocators wait for the runtime port. |
| B4 | base.md §§8.9, 9.1–9.8, 14.3, 15, 17 | Interface views, atomics, `volatile`, `asm`, attributes, `extern`, variadic C calls, `fmt`, `location()`, `export` | Partial (2026-09-04): interface views (§14.3) are two-word values formed from `T*` (or `const T*` when nothing mutates) that dispatch through a static witness table, a global of thunk addresses (`MirInitializer.FunctionAddress`) called with `CallIndirect`; both oracles, Wasm, and native QBE agree, and a `mutating` call writes the conformer back through the view. `volatile` (§15.2): every scalar access through `volatile T*`, the pointee or a field of it, is a canonical `VolatileLoad`/`VolatileStore`, which QBE legalizes through the relaxed `__atomic_load_N`/`__atomic_store_N` calls of §19.3 and Wasm and the oracles perform as plain accesses. atomics (§15.1): `@T` for integers, `bool`, pointer-width integers, and pointers (`TypeForm.Atomic`, stored as `T`), plain reads and `=` as sequentially consistent loads and stores, `+=`/`-=`/`|=`/`&=`/`^=` as wrapping read-modify-writes, and the methods `load`, `store`, `add`, `sub`, `set`, `clear`, `flip`, `swap`, and `cas` with `.relaxed`…`.seq_cst`, lowered to canonical `AtomicLoad`/`AtomicStore`/`AtomicUpdate`/`AtomicCompareExchange` that QBE bridges through the `__atomic_*` library and the oracles and Wasm perform plainly. Atomic `max`/`min` are `AtomicUpdate` operations that QBE writes as a load and compare-exchange loop in the type's signedness, and an array element indexed through a `volatile` pointer is a volatile access; an optional view `Writer?` is the view on the null-data niche, two words with `none` as a null data pointer (§5.6). `atomic.fence(.order)` from the standard `atomic` module is the canonical `Fence` instruction, `__atomic_thread_fence`/`__atomic_signal_fence` under QBE. `export func` (§17.6) is the Base spelling, and an export may name Base pointers, `T*?`, integer-backed enums, and Base structs by value, each published in the C header under its own name with the one null check at the boundary. `extern func` in Base takes the same boundary types plus `cstr` (`const c.char*`), so Base calls libc directly under QBE with the one null check at the boundary. Variadic calls (§17.2) promote their extras as C does and cross QBE with the `...` marker, `as "symbol"` binds a C name, and the standard `c` module carries the alias types of §5.2. Exported methods cross as `Type_method` with `self` first as `const Type *` or `Type *`. Spans in parameter position cross as `T *, size_t`, `func` types as C function pointers, and a fallible export takes the `int` status form with a final out-pointer. `extern union` declares overlapping C layout, and `bool` promotes to `int` in a variadic position. A Base package without `main` builds with `--lib` to a library, one relocatable object beside its `--c-header` header, and a C program links against both (§17.6, §19.6). `print(f"...")` writes each text piece and field to standard output through the standard `io` module's display functions (`write_signed`, `write_unsigned`, `write_bool`, `write_char`, `write_text`, `write_pointer`) with no string value formed; `io.lucb` declares `Writer`, `stdout()`, and `stderr()` over the `Builtin.write_output`/`write_error` intrinsics a standard Base module may use, and `u32(c)` is the checked conversion from `char` (§5.5, §14.4, §16.6). `writer.write(f"...")` on an `io.Writer` and `format(buffer, f"...") -> str!` (through `io.BufferWriter`, failing with `io.full`) are one fallible sequence of those display calls, so `try`/`catch` see the first failed write. `c.char`, `c.long`, `c.ulong`, and `c.wchar` are distinct target-width integers (§5.2): the backend fixes the width, constants must fit every target, widening is admitted only where no target loses a value, and the header spells them `char`, `long`, `unsigned long`, `wchar_t`. `cstr` is `const c.char*`, and `usize(n)`, `u32(size)`, `c.long(n)` and every other checked conversion involving a target-width integer bound the value against the destination's width at run time and trap out of range (§7.5). `location()` answers the caller's `Location` (`file`, `line`, `function`, the standard `io` module's struct), also as a parameter default evaluated at each call site (§9.1). Float fields display as the shortest round-tripping decimal through `io.write_float64`/`write_float32` (Ryū in Base), and `value.bits()` reads a float's IEEE pattern and `f64.bits(n)`/`f32.bits(n)` build one, `char(n)` is the checked conversion to a scalar value (§7.5, §14.4); `str(bytes)` and `str(text)` validate UTF-8 through `io.text_from_bytes`/`text_from_c` (§5.5); `--lib` merges the fence shims into the object with `ld -r`. `fmt` parameters carry the caller's formatted string as a span of `io.Piece` values on the caller's stack, printed, written, formatted, or forwarded by the callee (§9.1). Open: `asm`/`naked`, atomic `wait`/`wake` (they wait for the runtime's futex, B5), and the Wasm startup shim for a Base `main`; declaration attributes (§9.8) are one `SymbolAttributes` value on the syntax tree, `HirFunction`/`HirGlobal`, and `MirFunction`/`MirGlobal`: `inline`, `noinline`, and `cold` are hints QBE and Wasm ignore, `used` is a pruning root in the MIR optimizer, `section("name")` becomes QBE `section` linkage, and `weak` (on an exported function) and `used` become the assembler directives the QBE toolchain appends for Mach-O and ELF; `naked` waits for `asm` and `weak var` for exported globals, each with a diagnostic. |
...
| B6 | base.md §§16.5–16.6, 18 | The standard library in Base with the §18 crossing rules and `--costs`; automatic standard-module loading | Partial (2026-09-04): a package with `.lucb` sources finds the standard modules under `src/standard` (or `LUCE_STANDARD_ROOT`) without `--standard` flags; the library itself depends on B5; closes S12 and S30. |
...
| --- | --- | --- |
| `full/hello.luc` | Native entry and terminal output | Native QBE product smoke; output still needs an explicit captured assertion. |
...
| `full/stage0_calculator.luc` | Byte scanning, recursion, tuples, failure | Native QBE execution/status. |
| `full/stage0_sort.luc` | Fixed arrays, copies, mutation, sorting | Native QBE execution/status. |
| `full/stage0_brainfuck.luc` | Bytes, nested flow/match, bounded interpreter | HIR, MIR, Wasm, and native QBE execution. |
| `full/function_values.luc` | Exact named function values and indirect calls | HIR, MIR, Wasm, and native QBE execution. |
| `full/generic_functions.luc` | Unconstrained generic functions, inference, defaults, recursion, contextual values, and memoized concrete instances | HIR and MIR oracles plus Wasm and native QBE execution. |
| `full/generic_methods.luc` | Owner/method scope separation, suffix inference and explicit arguments, owner-dependent constraints, defaults, recursion, mutation, and concrete/generic struct, class, and enum receivers | HIR and MIR oracles plus Wasm and native QBE execution. |
| `full/generic_construction.luc` | Custom generic-struct initialization and inferred, contextual, and explicit generic struct/class/enum type functions | HIR and MIR oracles plus Wasm and native QBE execution. |
| `full/constrained_generics.luc` | Generic interfaces, explicit and generic-struct conformance, static requirement dispatch, concrete existential witnesses, and infallible-to-fallible requirement adaptation | HIR and MIR oracles plus Wasm and native QBE execution. |
| `full/generic_enums.luc` | Inferred/explicit/contextual generic enum applications, concrete methods and conformances, matching, existential dispatch, distinct payload shapes, and named-payload evaluation order | HIR and MIR oracles plus Wasm and native QBE execution. |
| `full/generic_classes.luc` | Inferred/explicit/contextual generic class applications, distinct managed payload/lifecycle identities, weak recursive edges, fallible construction, conformances, and existential dispatch | HIR and MIR oracles plus Wasm and native QBE execution. |
| `full/types_and_bindings.luc` | Transparent aliases, local/contextual inference, tuple binding, value copies, shared reference identity, recursive indirection, and nested generic mutable places | HIR and MIR oracles plus Wasm and native QBE execution. |
...
| `full/iteration.luc` | Compiler-known infallible/fallible iteration through concrete, constrained-generic, and existential values, including lexical iterator cleanup | HIR and MIR oracles plus Wasm and native QBE execution. |
| `full/hashing.luc` | Compiler-derived structural constraints and execution-local hashing across immutable value families | HIR and MIR oracles plus Wasm and native QBE execution. |
| `full/closures.luc` | Managed copied/shared/weak captures, fallibility lifting, nested escape, collection/field ownership, and weak-self cycle breaking | HIR and MIR oracles plus Wasm and native QBE execution. |
| `full/cfunc_values.luc` | Exact C-callable values and adapters | HIR, MIR, Wasm, and native QBE execution. |
| `full/conditional_binding.luc` | Optional conditional binding, branch-only payload scope, and absent fallback | HIR, MIR, Wasm, and native QBE execution. |
| `full/numeric_conversions.luc` | Checked integer/float construction, binary32 contextual rounding, width conversion, and truncation | HIR and MIR oracles plus native QBE and Wasm execution. |
| `full/array_slices.luc` | Immutable owned snapshots from inline fixed arrays, including escaping managed elements | HIR and MIR oracles plus native QBE and Wasm execution. |
...
| `full/maps_and_sets.luc` | Insertion-ordered map/set identity, lookup, mutation, copy, recursive equality, managed ownership, and alias-wide iteration invalidation | HIR and MIR oracles plus native QBE and Wasm execution and mutation traps. |
| `full/tasks.luc` | Named isolated workers, immutable graph transfer, ordered/cached observation, and lexical supervision | HIR and MIR oracles plus native QBE execution; Wasm emits an explicit unsupported-target diagnostic. |
| `full/bytes.luc` | Immutable owned byte concatenation/comparison and ownership-retaining escaping slices | HIR and MIR oracles plus native QBE and Wasm execution. |
| `full/strings.luc` | Ordinary/raw/triple text and bytes, owned UTF-8 concatenation/discard, Unicode scalar length/iteration, and scalar-preserving ordering | HIR and MIR oracles plus native QBE and Wasm execution. |
| `full/formatted_strings.luc` | Builtin/concrete/generic/existential `Display`, nested affine construction, triples/braces, Unicode, integer extrema, and both IEEE widths | HIR and MIR oracles plus native QBE and Wasm execution. |
| `full/traps.luc` | Dynamic nonrecoverable diagnostics, `never` callables, nested eager-prefix termination, and skipped deferred cleanup | HIR and MIR oracles plus captured native QBE and Wasm failure diagnostics. |
| `full/assertions.luc` | Default/dynamic assertion messages, successful continuation, and failed no-cleanup termination | HIR and MIR oracles plus captured native QBE and Wasm failure diagnostics. |
| `full/language_tour.luc` | Broad 1.0 declaration/control/managed surface | Parser only; each section migrates into focused executable examples as it lands. |
| `full/operators_and_literals.luc` | Literal, collection, type, and operator surface | Parser only beyond the already executable scalar subset. |
| `checkout/` | Multi-module application shape | Parser only until collections/strings are complete. |
| `full/c_api.luc` | C export surface | HIR/MIR/QBE plus a real linked C round trip; the native CLI also emits its checked C11 header and ABI report. |
...
| `base/spans.lucb` | `T[N]` arrays and `T[]` spans: checked indexing and slicing, `length`/`data`, spans from parts, `for` over both | HIR execution and a freestanding Wasm build through the pipeline; the same fixtures run natively under QBE (base.md §5.4, §8.3). |
| `base/flags.lucb` | `enum Access as u32:` flags: `\|` `&` `^` `~`, `(u32)mode`, checked `Access(n)`, C's `(Access)n`, `match` with `_` | HIR execution and a freestanding Wasm build through the pipeline; the same fixture runs natively under QBE (base.md §10.3). |
| `base/unions.lucb` | `union Word:` members at one address: write an integer, read its bytes or halves, write a float and read its bits, zero as bytes, `sizeof` | HIR execution and a freestanding Wasm build through the pipeline; the same fixture runs natively under QBE (base.md §10.4). |
| `base/globals.lucb` | module `var` zero or constant-initialised, `thread_local var`, a struct, an array, and the address of another global as static data, `pub let` | HIR execution and a freestanding Wasm build through the pipeline; the same fixture runs natively under QBE (base.md §6.3, §6.4). |
| `base/control.lucb` | `rows: for` with `break rows`/`continue rows`, and `.line(length) if length > 10:` guarded arms | HIR execution and a freestanding Wasm build through the pipeline; the same fixture runs natively under QBE (base.md §8.4, §8.5). |
| `base/errors.lucb` | `error(code, "text")` with a view message, `try`, `catch failure:` reading `code` and `message`, `recover`, `else error(...)`, `errdefer` beside `defer` | HIR execution and a freestanding Wasm build through the pipeline; the same fixture runs natively under QBE (base.md §8.8, §11.2–11.4). |
| `base/layout.lucb` | `packed struct`, `align(32) struct`, `align(16)` on a field, with `sizeof`/`alignof`/`offsetof` answering under each backend's rules | HIR execution and a freestanding Wasm build through the pipeline; the same fixture runs natively under QBE (base.md §5.11). |
| `base/views.lucb` | `let shape: Shape = &square` views, dynamic `area()`/`grow()` calls through witness tables, a `Shape[]` span of views | HIR execution and a freestanding Wasm build through the pipeline; the same fixture runs natively under QBE (base.md §14.3). |
| `base/volatile.lucb` | `volatile Timer*` field stores and compound updates, `volatile u8*` reads and writes | HIR execution and a freestanding Wasm build through the pipeline; the same fixture runs natively under QBE through `__atomic_*` calls (base.md §15.2, §19.3). |
| `base/atomics.lucb` | `@u64` counters with `+=`, `add(1, .relaxed)`, `max`, `min`, `swap`, and `cas`, an `@bool` flag with `store`/`load` orders, `atomic.fence` | HIR execution and a freestanding Wasm build through the pipeline; the same fixture runs natively under QBE through `__atomic_*` calls (base.md §15.1, §19.3). |
| `base/attributes.lucb` | `inline`, `cold noinline`, `used var`, and `weak export c func` on Base declarations | HIR execution and a freestanding Wasm build through the pipeline; the same fixture runs natively under QBE with the `.weak_definition`/`.no_dead_strip` directives appended (base.md §9.8). |
| `base/main.lucb` | `pub func main(arguments: str[]) -> i32` calling `printf` with the command line | Checked through the pipeline and built natively under QBE with Base's startup shim, then run with two arguments (base.md §9.7, §17.2). |
| `base/format.lucb` | `print(f"...")` over integers, `bool`, `char`, `str`, and `u32(letter)`; `format(buffer, f"...")` into a stack buffer; `io.stderr().write(f"...")` | Built natively under QBE and run: each piece reaches its sink through the standard `io` module's display functions, with no string value formed (base.md §5.5, §14.4). |
| `base/exports.lucb` | `export func` over `Point*`, `const Point*?`, a `Point` by value, and an `Access as u32` enum | HIR execution through the pipeline; the same signatures are called from a C `main` under QBE and rendered in the generated header (base.md §17.6). A struct by value in an export has no Wasm C ABI yet, and the Wasm build says so. |
| `base/allocation.lucb` | `try new Node(...)`, `new i64[3]`, `alloc u8[16]`, `alloc(8, 8)`, `free`, and `with` over a `memory.FixedBuffer`, with `memory.exhausted` caught when the buffer runs out | HIR execution beside the standard `memory` module and a freestanding Wasm build through the pipeline; the same fixture runs natively under QBE (base.md §12). |
| `base/pointers.lucb` | `T*`, `const T*`, `&place`, `*p`, and `p.field` in a Luce Base module | HIR execution through the frame-place pointer model and a freestanding Wasm build through the pipeline; the same fixtures run natively under QBE (base.md §5.3, §6.6, §7.7). |
| `base/c_arithmetic.lucb` | C's `//` and `%`, implicit same-signedness widening, the `+% +| +?` overflow family, and `(T)x` casts in a Luce Base module | HIR execution and a freestanding Wasm build through the pipeline; the same arithmetic floors in a `.luc` module (base.md §7.2, §7.5). |
