...
| §21 | C/native imports and exports, ownership/nullability, raw native source | complete | partial | partial | partial | partial | Scalar/handle/nested external and exported structs, strings, borrowed dense lists, anonymous `foreign` data pointers, incoming bare/nullable cfunc pointers, and explicit inbound-memory copies retain target-neutral HIR/MIR shape, with real QBE/C proofs. Standalone C11 export products execute through the CLI. Clang emits versioned FIIR plus generated nominal fundamental, scalar-typedef, and sign-magnitude enum carriers; constant-only anonymous enums, Clang-evaluated header-local fundamental-integer/Boolean/exact-IEEE objects, and explicitly selected fundamental-integer macros become typed constants. Exact floating encodings preserve finite values, infinities, and signed zero while canonicalizing unobservable NaN payloads. Declaration-only external scalar and enumeration objects stay live behind generated fixed-carrier readers and mutable writers; const and volatile remain C-product facts. Checked C adapters compile, link, run, assert exact scalar representations and constant semantics, reject target-range overflow, and reject undeclared enum values without target facts entering HIR/MIR. IEEE binary16 `_Float16` remains semantic `f16` across generated source and uses an exact private `f32` adapter carrier. Other extended floats and broader declarations, recipes, and callback runtime/lifetime rules remain. |
...
| S21 | §§21.1–21.5, §21.12, §§21.14–21.15 | FIIR, C import and wrapper/thunk generation, recipes, provenance, and support diagnostics | Export-side C11 products are complete. Import-side C `_Bool`, exact IEEE binary16 `_Float16`, exact IEEE binary32 `float`, exact IEEE binary64 `double`/`long double`, every fundamental C integer family, scalar typedef chains, named/typedef-backed C enums, constant-only anonymous enums, Clang-evaluated header-local fundamental-integer/Boolean/exact-IEEE objects, explicitly selected fundamental-integer macros, declaration-only external fundamental-scalar/enumeration objects, and plain nested records pass Clang facts → validated/serialized FIIR → generated raw/C products → both semantic oracles and real QBE/C execution. Scalar families and typedef names remain nominal; `_Float16` uses an exact private f32 carrier; integer/enum/macro constants use target-neutral sign-magnitude carriers while Boolean and floating constants retain their semantic kind and exact IEEE bits; external storage uses fixed-carrier accessors; records use logical fields while layout stays in FIIR/C. Extended `long double` fails rather than narrowing. Integer target-range failure traps before C, while alternate primitive/typedef targets, enum representation flags, object qualifiers, and record layout facts preserve identical Luce semantics. Other extended floating carriers, unions/bit-fields, enumeration object constants, non-integer macro constants, aggregate/atomic/thread-local objects, ownership recipes, support tiers, and regeneration diagnostics remain; C++ §§21.6–21.11 and §21.13 are deferred past 1.0. |
