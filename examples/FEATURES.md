...
| B2 | base.md §§5.3–5.5, 6.6, 7.7, 8.3 | Pointers, qualifiers, the null niche, spans, `str` views, `cstr`, address-of and the escape rule, pointer arithmetic and ordering, `for x in &items` | Partial (2026-09-03): `T*`, `const T*`, `&place` with the path-rooted qualifier, `*p` loads and stores, `p.field` auto-dereference, implicit `T*` to `const T*`, and address equality check, run through the reference interpreter's frame-place model, lower to canonical `Ptr`, and execute on QBE and Wasm; `examples/base/pointers.lucb` checks, runs, and builds. Open: the `T*?` niche, `void*`, pointer arithmetic and ordering, arrays and spans, `str` views, `cstr`, the escape rule. |
