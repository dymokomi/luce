...
- **`extern struct` is C layout, crossing by pointer** in both directions; by-value aggregates wait for generated shims (§21.17). `cfunc(params) -> R` is C's function pointer: capture-free functions convert to it, struct fields and results of that type are callable, and the ARC-carrying trampoline is the generator's machinery, not the language's (§21.19).
