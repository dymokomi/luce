...
- A callback arriving on a foreign thread is attached to the runtime only long enough to copy validated sendable arguments into the owning worker's ingress queue; ordinary Luce identity is never touched concurrently.
- A foreign API requiring an immediate return from an arbitrary thread accepts only a generated capture-free, non-effectful C-compatible function or a handwritten audited native adapter.
