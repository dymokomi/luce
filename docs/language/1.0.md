...
their format. FIIR-proven IEEE binary32 `float` uses `c.float` over `f32`;
IEEE binary64 `double` uses `c.double` over `f64`; and a `long double` with
that same exact binary64 model uses distinct `c.long_double` over `f64`.
Other C floating models require a generated lossless carrier and may not be
silently narrowed to a Luce float. Until such a carrier is implemented, binding
generation rejects the type before HIR construction.
...
the acyclic typedef chain and declaration origin; the carrier is always one of
`bool`, `i64`, `u64`, `f32`, or `f64` according to the lossless value domain.
