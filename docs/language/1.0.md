...
2. At the backend boundary that typed request is legalized to the private
   runtime ABI `luce_rt_alloc(byte_count, byte_alignment)`. Checked
   multiplication and layout come from that backend's existing type-layout
   cache. The returned block has one strong storage owner; retain/release share
   it, and the final release returns the opaque block to `libluce_rt` after its
   element owner has performed any required element destruction.
