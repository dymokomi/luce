...
`assert` traps when its Boolean condition is false. Assertions are never removed in a way that changes memory safety; release profiles may omit assertions explicitly marked as diagnostic-only through a testing/library API, not through a general language attribute. An assertion's condition must not have side effects: a program's behaviour is the same whether or not a diagnostic-only assertion is evaluated, and the compiler rejects a condition that calls anything it cannot prove effect-free once operational summaries exist.
