...
The pointer surface is typed and closed. `native.load`, `native.store`, typed
element advance, and typed copy retain the pointee `TypeId` until canonical
MIR; the backend alone computes byte size, alignment, and offsets. There is no
pointer-to-integer conversion, address ordering, untyped offset, or
source-visible `sizeof`/`alignof`. A conversion between native pointer types is
a named intrinsic checked by the compiler, never an ABI-layout assertion made
in ordinary source.
