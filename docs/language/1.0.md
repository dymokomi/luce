...
C Boolean, fundamental integer, exact floating, or scalar-typedef types remains
live storage. FIIR records its declared type, read-only versus read-write
access, volatility, and source origin. The generated raw module exposes an
ordinary reader function and, unless the object is `const`, a writer function.
Integer writers perform the same target-range check as function arguments.
The generated C product statically verifies the object's exact type and
qualifiers and is solely responsible for each actual load or store. HIR and
canonical MIR therefore contain only calls with fixed logical carriers—never
a C object size, alignment, storage address, or volatile operation. Atomic,
thread-local, aggregate, pointer, array, and definition-bearing objects require
their own complete contracts and fail import rather than being approximated.
