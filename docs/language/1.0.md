...
FIIR records declarations, layouts, calling convention, nullability,
mutability, exceptions, target conditions, and source origin as facts from the
foreign frontend. A separate recipe section records reviewed ownership,
lifetime, and failure relationships that the header cannot prove. Generators
consume FIIR to produce ordinary Luce modules; those modules then enter the
same frontend, HIR, and MIR pipeline as handwritten source. No foreign type,
pointer layout, or target ABI is added to HIR or MIR. LLVM is not required to
understand C++.
...
initializer. Supported Boolean and exact-IEEE binary16, binary32, and binary64
objects enter disjoint FIIR value kinds. Floating values retain the exact C
encoding through inspection and serialization; raw Luce reconstructs the
semantic `f16`, `f32`, or `f64` value, preserving finite values, infinities,
and signed zero while mapping every NaN payload to Luce's single observable
NaN. The generated C product reasserts the exact declared type and semantic
value, including the sign of zero and the NaN/infinity classes. Volatile
objects remain runtime storage even when `const`; unsupported extended
floating formats fail instead of narrowing.

A header-local `static const` object whose unqualified type resolves to a named
or typedef-backed C enumeration retains that declared nominal type and an
enumeration-specific sign-and-magnitude value in FIIR. The value domain stays
open: Clang supplies the semantic sign and defined `uint64_t` conversion, so a
valid value without a corresponding enumerator is preserved rather than
rejected or reclassified as an integer constant. Generated Luce constructs the
existing nominal enum carrier, while the generated C product independently
asserts the object's exact declared type and semantic value. No target enum
width or compatible integer type enters HIR or canonical MIR. Other object
kinds fail instead of masquerading as supported constants.

A declaration-only external object whose resolved type is one of the supported
C Boolean, fundamental integer, exact floating, enumeration, or scalar-typedef
types remains live storage. FIIR records its declared type, read-only versus
read-write access, volatility, and source origin. The generated raw module
exposes an ordinary reader function and, unless the object is `const`, a writer
function. Integer writers perform the same target-range check as function
arguments. Enumeration accessors reuse the sign-and-magnitude carrier and
reject an undeclared value on either a read or write; a target-selected enum
width never becomes shared representation. The generated C product statically
verifies the object's exact type and qualifiers and is solely responsible for
each actual load or store. HIR and canonical MIR therefore contain only calls
with fixed logical carriers—never a C object size, alignment, storage address,
or volatile operation. Atomic, thread-local, aggregate, pointer, array, and
definition-bearing objects require their own complete contracts and fail import
rather than being approximated.

An object-like macro enters the same scalar constant model only when the
binding request explicitly selects its name. Clang preprocessing establishes
the final active definition and its source origin; function-like, undefined,
and duplicate selections are rejected. Separate Clang-evaluated type and value
probes preserve typedef spelling, require a constant value, and classify the
result as Boolean, fundamental integer, supported exact-IEEE floating, or
named/typedef-backed enumeration. Integers cover the complete
signed-`i64`/unsigned-`u64` union; floating values retain their exact encoding;
enumerations retain their open sign-and-magnitude domain. `_Float16` format
facts are evaluated whenever a selected macro reaches that type. Luce never
interprets replacement text as if it were C semantics. Import order follows
the explicit selection order, and FIIR records whether every standalone
constant came from an anonymous enumeration, object, or macro. The generated C
product statically verifies every imported constant's type and semantic value
against the included header. Pointer, array, and string macros require storage
and lifetime carriers and fail import until those contracts are implemented.
...
A direct pointer to a typedef-backed incomplete C `struct` may enter the raw
layer as one pointer-shaped nominal `extern type`. FIIR retains the incomplete
record identity, exact C pointer spelling, declaration origin, header-proven
nullability, and pointee `const` qualification; it carries no size, alignment,
field, pointer-width, or ABI-placement fact into generated Luce, HIR, or MIR.
`_Nonnull` produces a bare raw handle and `_Nullable` produces an ordinary
optional handle. Missing or `_Null_unspecified` nullability is conservative:
the raw result or parameter is optional rather than silently promising a
non-null C value. The generated C adapter alone converts between the nominal
carrier and the typed C pointer.

This first raw-handle rung does not infer ownership or lifetime from C syntax.
Those concepts are absent from the Clang boundary value rather than recorded
as fabricated `unspecified` declaration facts; a separate binding recipe must
supply them before a safe wrapper can be generated.
Pointee mutability is an auditable foreign fact, not yet a safe-source
capability: the generated raw module is native-authority code and a reviewed
safe wrapper must preserve or strengthen it. Direct `struct Tag *` spellings,
pointer typedefs, pointer-to-pointer/out handles, general object pointers,
pointer record fields, and broader ownership relationships remain rejected
until the corresponding recipe and carrier rows below are implemented. Function and array pointer
declarators are rejected as their own shapes rather than being parsed as
scalar or opaque-record results.

### 21.4 C ownership and nullability

C headers rarely contain enough lifetime information. A declarative binding
recipe supplies facts the header cannot. The implemented 1.0 recipe vocabulary
is a closed TOML subset; unknown tables, keys, relationships, duplicate facts,
or references to declarations and parameters that do not exist are errors.
Each table applies to one imported function:

```toml
[[native.temperature.function]]
name = "sensor_open"
result = "owned via sensor_close"
...
name = "sensor_alias"
parameter.0 = "borrowed for call"
result = "borrowed from parameter.0"

[[native.temperature.function]]
name = "sensor_validate"
parameter.0 = "borrowed for call"
result = "status"
success = 0
error-code = 31
message = "temperature sensor is invalid"
```

For the current direct-opaque-handle rung, every opaque parameter must say
`borrowed for call`, except the one parameter of a named disposer. A disposer
is implied by its owning acquisition and cannot have a separate recipe. Every
opaque result must be either `owned via NAME` or `borrowed from parameter.N`. A
disposer must take exactly one non-null handle of the same opaque type and
return `void`. A returned borrow must name a non-null, borrowed-for-call
parameter of the same opaque type; the safe value retains that parameter's
owner. Every borrowed handle type must have at least one owned acquisition in
the same recipe, so the generated safe module never refers to an owner it
cannot construct. A `status` result must be a C integer and supplies one
representable success value plus a package error code and nonempty stable message. Error
codes may not collide within one recipe; ordinary package checking rejects
collisions with other generated or handwritten errors.

The generator validates and embeds the recipe, with source origins, in the
versioned FIIR product. It then produces an ordinary safe Luce module in
addition to the raw native module and C adapter. Only functions named by the
recipe enter this reviewed safe surface; all other imported declarations remain
available solely through the raw native module. The safe module provides:

- `T?` for nullable pointers;
- owner classes with idempotent `close`/`deinit` for acquired resources;
- checked borrow capabilities that retain their owner and trap after explicit
  close, including returned borrows anchored to an input owner;
...
The first three bullets and integer-status conversion are implemented for direct
opaque handles. The remaining array/string/callback carriers are later rows in
the same FIIR dependency sequence and stay rejected until their complete
relationships exist. Raw access remains available only in audited
`.lucn` modules (§21.12). Missing ownership facts are a generation error
for safe wrappers, never a guessed convention. `luce bind` accepts
`--recipe PATH --safe PATH` only as a pair, so raw-only generation remains
explicit and safe generation cannot silently omit its reviewed input.
...
Low-level operations are confined to audited `.lucn` modules, normally generated. They may use compiler-known types:
...
The suffix is module authority, not a lexical mode. A `.lucn` module
...
available to application packages, including their `.lucn` modules.
...
`.lucn` module while compiling the sealed runtime package. A successful
...
application declaration, and does not grant `.lucn` authority to the
