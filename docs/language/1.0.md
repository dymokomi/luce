...
C-adapter assertions rather than HIR or MIR facts. Macro and object constants
enter this same typed value model only after Clang has evaluated them; the
importer never interprets preprocessor replacement text as if it were C
semantics. The generated C product statically verifies every imported
constant's type and value against the included header.
