...
Both may declare methods/type functions and implement interfaces. Structs store a product of named fields; enums store one case, optionally with a payload. Luce deliberately has no second ordinary `union` declaration: a payload-free enum and a tagged union are the same closed-sum concept with the same construction, matching, visibility, layout, generic, and compatibility rules. Raw C unions remain a native-boundary concern.
