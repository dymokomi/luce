...
`cfunc(params) -> R` is C's function pointer as a Luce type. The word is **contextual, not a keyword**: `cfunc(` is recognized only in type position, and programs keep the identifier (§28.5).
