...
The repository currently proves 1132 compiler tests across 63 files, plus the
...
- [ ] **B1 — the Base profile.** *Landed so far (2026-09-03, `done.md` §2):
  ...
  rejections, the freestanding check, the Base-only tokens and reserved
  ...
  wrapping/saturating/checked operator family, and `(T)x` casts between
  numbers; pointer/enum/text casts land with their types, and folding
  `usize` constants into array lengths is open.* Select the profile by the `.lucb` suffix
...
- [ ] **B2 — pointers and spans.** *Landed so far (2026-09-03, `done.md` §2):
  ...
  the escape rule, and `str` as a two-word view iterated by scalar;
  `cstr` waits for the `c` module (B4).* `T*`, `const`/`volatile` qualifiers, the
  ...
  to MIR.
- [x] **B3 — bindings, globals, unions, allocation.** *Landed (2026-09-04,
  `done.md` §2): zero values and `---`; integer-backed enums, whose C
  header form waits for B4 (§17.6); unions, with the canonical `Union`
  type and byte-image oracles; module globals with constant initialisers
  folded into the canonical `MirInitializer` value tree; labeled loops,
  match guards, and `errdefer`; the Base failure model (§11.2–11.4) with
  `Error` text as a view; `packed`/`align` layout words; allocation (§12)
  over the standard `memory` module's `Allocator` interface with
  `FixedBuffer`, where `new`/`alloc` are one HIR `Allocate` node and
  `with` a `WithAllocator` scope, so no memory instruction was added to
  MIR. `PageAllocator`, `CAllocator`, `Arena`, and the §12.5 linter wait
  for B5's runtime port.*
  Zero values and `---`,
...
- [ ] **B4 — interface views, atomics, `asm`, calling C.** *Landed so far
  ...
  header (§17.6, §19.6); `print(f"...")` writing each piece to standard
  output through the standard `io` module's display functions, with `Writer`,
  `stdout()`, and `stderr()` in `io.lucb` over the two `Builtin` write
  intrinsics a standard Base module may name (§5.5, §14.4, §16.6);
  `writer.write(f"...")` and `format(buffer, f"...") -> str!` as one
  ...
  §14.4); `str(bytes)`/`str(text)` validated in `io.lucb` (§5.5);
  `f64.bits(n)`/`f32.bits(n)` and the checked `char(n)` (§7.5); `extern func` with those types, `cstr` (`const c.char*`),
  ...
  §15.1's `@Node*?` reads as an atomic pointer; the parser follows §21's
