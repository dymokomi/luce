...
- [ ] **Data-oriented layout — shape now, tuning at the measurement**: (1) ~~ids to `u32`~~ done; (2) ~~MIR as a flat instruction array with `Else`/`Case`/`Default`/`End` markers~~ done 2026-08-28 (`done.md` §2); (3) tiny tokens (`kind: u8, start: u32`, text recomputed) and spans as `u32` offsets with line/column derived; (4) operand lists (`results`, `arguments`, `values`) into a per-function operand array; (5) HIR/AST as flat node tables with an extra-data array — design note first, decide with the measurement whether the once-walked HIR tree is worth it. Generic structure-of-arrays is a library/tooling question (hand-written for the hot tables, or generated from `luce describe`), never a reason to add compile-time execution.
