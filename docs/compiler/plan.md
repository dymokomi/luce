...
| Lowerer (`lowerer.luc`) | Slices 3a–3c, milestone 4, and structs done: scalars and locals, control flow, calls, constants, tuples, optionals, `str`/`bytes` values and equality, structs, methods, `mutating`, field places, `print` of a value. Not yet: everything HIR does not generate. |
...
- [x] **Structs and methods** in `hir_gen` → lowerer → wasm: fields with `let`/`var`/`pub` and constant defaults, the synthesized memberwise initializer (spec §10.1 visibility rule), field access and `root.a.b = v` places, methods with `self`, `mutating` methods (the receiver must be a mutable place; lowered as a store through the pointer the aggregate protocol already passes), type functions, struct equality, structs inside tuples/optionals, cross-module `module.Struct`. Gate met: `HirStruct`/`HirField`/`HirFunction`/`HirModule` retain documentation, `HirParameter` retains `default_value`, and parameter defaults are embedded at call sites. **Deferred to the failure slice:** custom `init` (needs `unit!` and partially initialized `self`).
...
## 10. Cautionary tales — Bun's Rust rewrite and what we take from it

Read 2026-08-27: Bun's "Rewriting Bun in Rust", Andrew Kelley's reply, Loris Cro's "Fix your asserts" and "Contributor poker and AI", and Zig 0.15.1's time report. The two sides disagree about blame, not about facts; both sets of facts are lessons.

| What happened | Rule for Luce | Where it already stands |
|---|---|---|
| Bun's bug catalogue — use-after-free, double-free, leaks, races — clustered at the boundary between JavaScriptCore's GC and Zig's manual memory: "no language really designs for it". Ownership across that seam was a style guide, and "the challenge with style guides is enforcement". | Ownership across every seam is the compiler's knowledge, never a convention: generated C bindings carry ownership and nullability (spec §21.4), the wasm guest ABI passes only values and lengths, errors cross as data. The lineage's own version of this bug is the `thread_local const char*` borrow in Kinogaki's C ABI (§9.2). | failure-as-data in MIR; owned/scoped values are gates before classes and closures (§6) |
| Kelley: the "style guide vs language feature" framing hides the main way bugs are removed — engineering time spent finding them. Bun told ZSF they were not fuzzing while the post implied they were. | Never claim testing that is not running. The three-way harness (§2) is real and has found six bugs; generated programs and fuzzing are **not yet built** and are listed as future release gates, not as current practice. | §2, §6 self-hosting item |
| A `debug_assert!` with a side effect broke hot reload in release because Rust erases the argument; Zig's `assert` runs in every build. Cro: silently disabled asserts "gaslight you" — the program still misbehaves, you just don't hear it. | `assert` traps in every build (spec §13.8 already says so); add to the spec that an assertion's condition must be free of side effects, checked by the compiler once operational summaries exist. Checks are removed only by proof (gap audit §8), never by build mode; semantics are identical in every profile. | spec §13.8; MIR "semantics fixed" rule (§3) |
| Two of Bun's regressions were semantic drift during the port: `trunc` where `floor` was meant, and bounds checks that differed between debug and release. | The oracle rule: a semantics question is settled by the spec, implemented in the HIR interpreter first, and every other stage is made to agree. Floor division, `%`, shifts and overflow are already fixed in MIR and tested on all three executions. | §2 |
| `comptime` abuse made Bun's Zig build slow enough that Zig shipped a time report to audit it. Bun's binary-size and LTO wins were engineering that "should have been done in the Zig codebase since the beginning". | No compile-time execution in Luce (spec §15.4 limits stay); monomorphization budgets are part of the const-generic gate; a `luce build --time-report` is on the tooling list once generics exist. Whole-program MIR means LTO is free by construction; stack-slot reuse and lifetime facts belong to the optimizer, tracked like any cost. | spec §15.4; gap audit §5 |
| The rewrite ran 64 Claude instances with split-context adversarial review; 4% of the result is `unsafe`; Kelley calls the codebase "hacks on top of hacks" and the reviews "unreviewed". Cro: AI-driven drive-by PRs break contributor poker. | This repo is also written with AI, so the defences are structural: small commits at green milestones, the harness as the gate for every slice, readability audits of every file, the privilege ledger, and honest plan entries over claims. When the repo takes outside contributions, the policy is written before the first PR, not after. | §8 conventions; memory `commit-rules` |
| Bun's relationship with its upstream (ZSF) decayed into resentment on both sides. | Stage-0 is our upstream: every defect goes to that team as a reproduction with a `run.sh`, workarounds are removed when fixes ship, and the plan records what was fixed where. | §7 |
| Kelley on rewrites in general: the language was not the problem; values and effort were. | Luce's claim is not "a rewrite in a safer language fixes the lineage". It is that the seams (§9.2) are unenforceable in C++ and that a language can enforce them; the proof is the guest and host proving programs passing the harness, not a bug count in a blog post. | §9.3 |
