...
- [ ] **Decompose stable pass ownership before large files become permanent.**
  ...
  expression/place, and pattern/control collaborators. Do not simulate partial
  classes with forwarding wrappers or duplicate pass state merely to reduce a
  line count. Parser grammar is already frozen; separate its byte/token
  plumbing only where one owner can retain the cursor and diagnostic state.
