...
- [ ] **Close the dialect gap — our sources must be legal in both compilers at once.** Our tree is written in Stage-0's dialect, and two spellings differ from the 1.0 spec our own front end implements. File-scope `const` was closed on 2026-08-29 (0.25 took `let`; 247 respelled). The one still open is the argument label: Stage-0 writes `f(name = value)`, the spec (§8.2) and our parser write `f(name: value)`. Until Stage-0 accepts the spec form our source cannot be its own input, so this is the first item on the 0.26 request list, not something to work around here. Nothing else in the tree is known to differ; confirm by feeding our own sources to our own parser once the labels land.
...
- [ ] **Bound recursion the way shipping compilers do** — the four gaps in §8.2, in that order: a Swift-style nesting cap in the parser and `hir_gen` (256, fatal, named in the diagnostic); `frame_limit` derived at startup from queried stack bounds instead of declared; thinner interpreter frames; a stack reservation on the ELF path. The first is a correctness gap today, not a tuning question: nested source crashes the front end.
...
- **Indices, not pointers, across stages.** Every program-wide table (`types`, `symbols`, `structs`, `functions`, `data`, registers, slots, and — once §5 item 4 lands — HIR nodes) is addressed by a `u32` id into a flat list; no stage hands another a pointer graph. New tables follow this. Under Stage-0 an id is widened at the index (`items[i64(id.value)]`) because lists index with `i64` only; the noise goes away with the spec's index-type decision (collections slice), not with self-hosting.
- **Layout truth comes from C1, not Stage-0.** Whether Stage-0 stores a `list[struct]` with a union payload inline is unknown and does not matter: our own `make_struct_type`/`make_enum_type` lay them out inline, so the shapes chosen now become real memory when the compiler compiles itself. Three Stage-0 conveniences are worth requesting but block nothing: inline storage (for measuring before C1), any-integer indexing (after the spec decision), and `match` arms naming several union members.
...
Where we are short, in the order it will bite:

- **The front end has no nesting guard at all.** `parser.luc`, `hir_gen.luc`, and the analyzer recurse on nested expressions with nothing bounding them; `expression_limit` counts tokens, not depth. Deeply nested source crashes the compiler before the interpreter is ever reached — Zig's failure mode, and Swift's `MaxDepth = 256` with a `structure_overflow` diagnostic is the cheap, standard fix.
