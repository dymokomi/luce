...
| Lowerer (`lowerer.luc`) | Everything HIR generates: scalars and locals, control flow, calls, constants, tuples, optionals, `str`/`bytes` values and equality, integer ranges and `for`, structs, enums, `match`, methods, `mutating`, field places, `print` of a value. |
...
| Tests | 374 unit tests; CLI contract script; `wasmtime` execution script; native smoke script (arm64 hello + overflow trap). `tests/compiler/differential_test.luc` runs ~120 fixtures through HIR interpreter, MIR interpreter, and every encoder and requires agreement. |
