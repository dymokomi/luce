...
| Tests | 668 unit tests across 16 files, plus CLI, `wasmtime`, QBE differential, and host-native smoke gates. `tests/compiler/differential_test.luc` runs the complete non-trapping and trapping corpus through HIR, optimized MIR, and the QBE product toolchain and checks values, output, and traps. |
...
- [x] **Compiler-derived structural markers and immutable hashing**
  ...
  genuine shared function-emission owner replaces those helpers. Cycle-aware
  structural list equality remains prerequisite work for maps and sets.
