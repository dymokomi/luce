...
| Lowerer (`lowerer.luc`) | Everything HIR generates: scalars and locals, control flow, calls, constants, tuples, optionals, `str`/`bytes` values and equality, structs, enums, `match`, methods, `mutating`, field places, `print` of a value. |
...
- [x] **Ids are `u32`** (2026-08-28). `TypeId`, `SymbolId`, `ModuleId`, `RegisterId`, `FunctionId`, `ExternId`, `GlobalId`, `DataId` hold a `u32` index; `no_register` (`u32` max) replaces the `-1` sentinel. 95 index/comparison sites widen with `i64(...)` under Stage-0. The first data-oriented-layout item (`plan.md` §5).
