...
| Tests | 594 unit tests across 16 files, plus CLI, `wasmtime`, QBE differential, and host-native smoke gates. `tests/compiler/differential_test.luc` runs the complete non-trapping and trapping corpus through HIR, optimized MIR, and the QBE product toolchain and checks values, output, and traps. |
...
- [x] **Core managed closures preserve one target-neutral semantic shape**
  ...
  ownership evidence, direct-cycle diagnostics, and accidental-cell warnings
...
- [x] **Complete the executable closure conversion, storage, and cycle matrix**
  ...
  example cover the completed matrix. The non-fatal accidental shared-cell
  advisory waits for the common §24 diagnostic channel; sendability waits for
  workers.
