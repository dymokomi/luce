...
| Tests | 579 unit tests across 16 files, plus CLI, `wasmtime`, QBE differential, and host-native smoke gates. `tests/compiler/differential_test.luc` runs the complete non-trapping and trapping corpus through HIR, optimized MIR, and the QBE product toolchain and checks values, output, and traps. |
...
- [x] **Core classes preserve nominal identity until the backend boundary**
  ...
  closures trigger the next review because capture environments may establish
  a real independent owner. Remaining §11 work is tracked separately rather
  than hidden behind this core milestone.
