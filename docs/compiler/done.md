...
| Tests | 723 unit tests across 16 files, plus CLI, `wasmtime`, QBE differential, and host-native smoke gates. `tests/compiler/differential_test.luc` runs the complete non-trapping and trapping corpus through HIR, optimized MIR, and the QBE product toolchain and checks values, output, and traps. |
...
- [x] **Extern structs remain ordinary values until the C pointer boundary**
  ...
  executable scalar/handle/nested-extern vocabulary. Bare cfunc fields retain
  an explicit diagnostic until the dynamic foreign-pointer oracle exists;
  accepting a symbolic HIR function in their place would be a false model of
  a pointer supplied by C. Canonical MIR records no
