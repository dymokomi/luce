...
with tempfile.TemporaryDirectory(prefix='luce-public-imports-') as temporary:
    root = Path(temporary)
    write(root, 'helper/luce.toml', '[package]\nname = "helper"\n[exports]\nmeasure = "helper.measure"\n')
    write(root, 'helper/src/helper/measure.lucb', 'pub let invalid: ErrorCode = ErrorCode.package(1)\npub func length(text: str) -> i64:\n    return (i64)text.length\n')
    ...
    for flags in FLAGS:
        ...
        run(root / 'consumer')
print('PASS public Button imports, aliases, transitive package exports, standard math and relocation; six modes')
