...
def run(*command):
    return subprocess.run(list(map(str, command)), cwd=ROOT, capture_output=True, timeout=120)
...
def check(source):
    ...
    try:
        ...
        if fixture:
            ...
            with tempfile.TemporaryDirectory(prefix='luce-conformance-') as directory:
