...
grep -q '"kind": "opaque_handle"' "$test_dir/temperature.fiir.json"
grep -q '"kind": "owned"' "$test_dir/temperature.fiir.json"
...
grep -q 'pub func luce_temperature_sensor_validate(sensor: luce_temperature_sensor_borrow) -> unit!' "$test_dir/temperature/safe.luc"
...
grep -q '_Generic((LUCE_TEMPERATURE_DEFAULT_SCALE), luce_temperature_scale: 1, default: 0)' "$test_dir/temperature.adapter.c"
grep -q '_Static_assert(LUCE_TEMPERATURE_DEFAULT_SCALE == -INT64_C(2)' "$test_dir/temperature.adapter.c"
grep -q '_Static_assert(LUCE_TEMPERATURE_OPEN_SCALE == UINT64_C(7)' "$test_dir/temperature.adapter.c"
...
grep -q '(void \*)(luce_temperature_sensor_open((luce_degrees)value))' "$test_dir/temperature.adapter.c"
